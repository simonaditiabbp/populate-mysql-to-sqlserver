...
def sync_data_timbang():
    ...
    try:
        ...
        if not logs:
            print("Tidak ada log baru di DB PC", MYSQL_LOG)
            return
                ...
                if aksi in ('INSERT', 'UPDATE'):                    
                    ...
                    if exists:
                        ...
                        if changed_cols:
                            ...
                            mysql_cur.execute(
                                f"UPDATE {MYSQL_LOG} SET STATUS = 'SUCCESS', MESSAGE = '{MESSAGE_LOG}', COUNTER_DONE = %s, PC_NAME = %s WHERE NOURUT1 = %s AND PLANT_ID = %s AND AKSI = 'UPDATE' AND COUNTER_DONE = %s",
                                (row_counter_done_update, PC_NAME, NOURUT1, PLANT_ID, 0)
                            )
                            mysql_conn.commit()
                            
                        else:
                            MESSAGE_LOG = f"Tidak ada perubahan untuk {NOURUT1}-{PLANT_ID}."
                            print(MESSAGE_LOG)
                            mysql_cur.execute(
                                f"UPDATE {MYSQL_LOG} SET STATUS = 'FAILED', MESSAGE = CONCAT(COALESCE(MESSAGE, ''), ' | [Error Populate Data] : ', {MESSAGE_LOG}), COUNTER_DONE = %s, PC_NAME = %s WHERE NOURUT1 = %s AND PLANT_ID = %s AND AKSI = 'UPDATE' AND COUNTER_DONE = %s",
                                (row_counter_done_update, PC_NAME, NOURUT1, PLANT_ID, 0)
                            )
                            mysql_conn.commit()
...
def sync_data_timbang_log():
    try:
        ...
        if not logs:
            print(f"[{datetime.datetime.now()}] Tidak ada log baru untuk dikirim ke SQL Server.")
            mysql_conn.close()
            return
